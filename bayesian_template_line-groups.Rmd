---
title: "Bayesian template draft line chart with groups"
...
This template will produce a **line chart with groups**. If your analysis includes a **two-way ANOVA**, this might be the right template for you.
...
mydata <- read.csv("datasets/stigmatized_campaigns_simulated-data.csv") #CHANGE ME
mydata$x <- mydata$order #CHANGE ME
mydata$y <- mydata$persuasiveness #CHANGE ME
mydata$group <- mydata$attitude #CHANGE ME
...
x_lab = "Before/after support" #CHANGE ME
y_lab = "Persuasiveness" #CHANGE ME

# the default code for the plots - if needed, the animated plot aesthetics can be customized here
graph_plot <- function(data) {
  ggplot(data, aes(x = x, y = .value, col = group, group = group)) + #do not change
