...
mydata %>% 
  data_grid(x, group) %>%
  add_fitted_draws(m) %>%
  ggplot(aes(x = x, y = .value, fill = group, color = group)) +
  stat_lineribbon(alpha = 1/5) +
  coord_cartesian(ylim = c(0, max(mydata$y, na.rm=TRUE))) +
  geom_point(aes(y = y), data = mydata, position = position_dodge(width = 0.1))
```
...
mydata %>% 
  ...
  coord_cartesian(ylim = c(0, max(mydata$y, na.rm=TRUE))) +
  geom_line(aes(group = paste(group, .draw)), alpha = .2) +
  geom_point(aes(y = y), data = mydata, position = position_dodge(width = 0.1))
```
...
```{r}
n_lines = 100  # number of lines we want to show
...
```{r}
animate(
  p + geom_point(aes(y = y), data = mydata, position = position_dodge(width = 0.1)),
  nframes = n_lines * 2, fps = 10
)
```
