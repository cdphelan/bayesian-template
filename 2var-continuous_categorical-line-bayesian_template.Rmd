...
7. a_prior: Select the intercept (likely the control condition mean). 

8. a_prior_max: Select the maximum plausible value of the intercept (maximum plausible value of control condition data). (We will use this to calculate the sd of `a`.)
...
Next, you'll want to check your priors by running this code chunk. It will produce a set of 100 sample draws drawn from the priors you set in the previous section, so you can check to see if the values generated are reasonable. 
...
# this function is defined in 'plotting_functions.R', if you wish to customize the aesthetics.
prior_HOPs = animate(HOPS_plot_6(prior_draws), nframes = n_draws * 2, fps = frames_per_second)
prior_HOPs
```
...
fit = mydata %>%#pipe mydata to datagrid()
      data_grid(x1 = seq_range(x1, n = 20), x2) %>% #create a fit grid with each level in x, and pipe it to add_fitted_draws()
      add_fitted_draws(m) #add n fitted draws from the model to the fit grid

# Plot the posterior draws
# this function is defined in 'plotting_functions.R', if you wish to customize the aesthetics.
static_post_plot_6a(fit)
...
# this function is defined in 'plotting_functions.R', if you wish to customize the aesthetics.
post_HOPs = animate(HOPS_plot_6(p), nframes = n_draws * 2, fps = frames_per_second) 
post_HOPs
